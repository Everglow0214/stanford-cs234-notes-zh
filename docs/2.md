...
然后我们考虑算法 3 中的迭代，将它们定义为 ${V_k}_{k\geq 1}$。注意，它们满足下列关系：
...
根据 [**定理 A.5**](#thm_A5) 可知 ${V_k}_ {k\geq 1}$ 是一个柯西序列，因此根据 [**定义 A.1**](#def_A1) 可知存在 $N\geq 1$，使得对于所有 $m,n\geq N$，都有 $\Vert V_m-V_n\Vert_{\infty}<\epsilon$。