...
**练习 3.1** （a）证明 $\mathbf{P}$ 是一个行随机的（row-stochastic）矩阵；（b）证明 $1$ 是行随机的矩阵的特征值，并找到一个对应的特征向量；（c）证明行随机的矩阵的特征值的最大绝对值为 $1$。
...
$$
\lVert \mathbf{A}x\rVert_{\infty} = \mathop{\max}_ {i=1,...,m}(\sum_{j=1}^{n}|A_{ij}|)。
\tag{3}
$$