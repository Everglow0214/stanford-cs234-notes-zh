...
**练习 3.8** 考虑矩阵 $(\mathbf{I}-\gamma\mathbf{P})$。（a）证明：$1-\gamma$ 是该矩阵的特征值，并找到一个对应的特征向量；（b）对于 $0<\gamma<1$，应用 **练习 3.1** 的结论来证明 $(\mathbf{I}-\gamma\mathbf{P})$ 是非奇异的（可逆的）。

**练习 3.9** 考虑 **3.2.4** 小节中的马尔可夫奖励过程。（a）如果 $H$ 是无穷的，计算所有状态的值函数。