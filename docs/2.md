...
在无限时间范围的情况下，和状态值函数类似，奖励、转移概率和策略的不变假设意味着对于所有的 $s\in S$ 和 $a\in A$，$Q_i^{\boldsymbol{\pi}}(s,a)=Q_j^{\boldsymbol{\pi}}(s,a)$，这里 $i,j=0,1,\cdots$，因此在这种情况下，因此我们可以做以下定义：
...
**练习 3.13** 考虑策略 $\boldsymbol{\pi}=(\pi,\pi,\cdots)$。如果状态转移概率不变，并且奖励不变，在时间范围 $H$ 无限的情况下，使用（[22](#eq22)）和（[24](#eq24)）中的定义证明：对于所有的 $s\in S$ 和 $a\in A$，对于所有的 $i,j=0,1,\cdots$，（1）$V_i^{\boldsymbol{\pi}}(s)=V_j^{\boldsymbol{\pi}}(s)$；（2）$Q_i^{\boldsymbol{\pi}}(s,a)=Q_j^{\boldsymbol{\pi}}(s,a)$。

在无限时间范围的情况下，关于固定转移概率和固定奖励的假设使得我们可以推出一个重要的恒等式：
