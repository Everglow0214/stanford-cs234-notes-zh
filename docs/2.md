...
给定一个 MDP，这个 MDP 的策略指定在每个状态下要执行的动作。策略可以是确定的或随机的，我们将考虑概率分布式的策略来同时包含这两种情况。需要注意的是，策略可能会随时间而变化，尤其是在有限时间范围的 MDP 中。我们将用 $\mathbf{\pi}$ 来表示一个通用的策略，定义为无限维数组 $\mathbf{\pi}=(\pi_0,\pi_1,\cdots)$，这里 $\pi_t$ 表示在时间 $t$ 的策略。我们称不随时间变化的策略为固定策略（stationary policies）并将其表示为 $\pi$，即在这种情况下 $\mathbf{\pi}=(\pi,\pi,\cdots)$。对于固定策略 $\pi$，如果在时间 $t$，行为体处于状态 $s$，它将选择一个概率为 $\pi(a|s)$ 的动作 $a$，并且这个概率不依赖于 $t$；对于非固定策略，这个概率将依赖于 $t$，我们用 $\pi_t(a|s)$ 来表示。