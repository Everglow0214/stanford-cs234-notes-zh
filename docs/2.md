...
**练习 3.11** 假设在算法 3 中，初始化时 $V'$ 被随机设置（所有元素都是有限的）而不是被设置为 $V' \leftarrow 0$。（1）算法仍会收敛吗？（2）算法仍然保持和 [**定理 3.1**](#thm3_1) 相同的误差吗？

**练习 3.12** 假设 [**定理 3.1**](#thm3_1) 的假设成立，使用和 [**定理 3.1**](#thm3_1) 相同的符号证明：
