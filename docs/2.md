...
<span id='eq22'>$\bullet$状态值函数：对于状态 $s\in S$，状态值函数 $V_t^{\boldsymbol{\pi}}(s)$ 定义为在时间 $t$ 起始于状态 $s_t =s$，遵循策略 $\boldsymbol{\pi}}$ 的期望回报：$V_t^{\boldsymbol{\pi}}(s) = \mathbb{E}_{\boldsymbol{\pi}}[G_t|s_t=s]$，这里 $\mathbb{E}_{\boldsymbol{\pi}}$ 表示关于策略 $\boldsymbol{\pi}$ 的期望。通常在期望的表达式中我们省略 $\boldsymbol{\pi}$，因此除非特殊强调，$\mathbb{E}$ 将表示 $\boldsymbol{\pi}$：</span>
